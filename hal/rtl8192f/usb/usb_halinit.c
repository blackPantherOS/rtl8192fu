...
u8 SetHwReg8192FU(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	case HW_SET_GPIO_WL_CTRL: {
		...
		if(enable)
			rtw_write8(padapter, REG_SW_GPIO_SHARE_CTRL,
					rtw_read8(padapter, REG_SW_GPIO_SHARE_CTRL) | BIT_SW_GPIO_FUNC);
		else
			rtw_write8(padapter, REG_SW_GPIO_SHARE_CTRL,
					rtw_read8(padapter, REG_SW_GPIO_SHARE_CTRL) & ~BIT_SW_GPIO_FUNC);
...
static u8 rtl8192fu_ps_func(PADAPTER padapter, HAL_INTF_PS_FUNC efunc_id, u8 *val)
{
	...
	case HAL_USB_SELECT_SUSPEND: {
		...
		rtl8192f_set_FwSelectSuspend_cmd(padapter, bfwpoll, 500); /*note fw to support hw power down ping detect */
	}
	break;
