	...
	if (_FALSE == pHalData->bautoload_fail_flag) {
		...
		*		if (IS_BOOT_FROM_EEPROM(padapter)) */
		if (_TRUE == pHalData->EepromOrEfuse) {
			...
			_rtw_memcpy((void *)PROMContent, (void *)pHalData->efuse_eeprom_data, HWSET_MAX_SIZE_8192F);
		}
	} else {
		/* autoload fail */
...
static void fill_txdesc_vcs_8192f(PADAPTER padapter, struct pkt_attrib *pattrib, u8 *ptxdesc)
{
	...
	if (pattrib->vcs_mode) {
		...
		SET_TX_DESC_RTS_RATE_8192F(ptxdesc, 8); /* RTS Rate=24M */
...
Hal_ReadRFEType_8192F(PADAPTER Adapter, u8 *PROMContent, BOOLEAN AutoloadFail)
{
	...
	if (!AutoloadFail) {
		if (GetRegRFEType(Adapter) != CONFIG_RTW_RFE_TYPE) {
			...
			pHalData->rfe_type = PROMContent[EEPROM_RFE_OPTION_8192E] & 0x1f; /* 0xCA[4:0] */
		}
	} else {
		if (GetRegRFEType(Adapter) != CONFIG_RTW_RFE_TYPE) {
			pHalData->rfe_type = GetRegRFEType(Adapter);
	...
	u8 *pbuf)
{
	...
	if (pxmitframe->frame_tag == DATA_FRAMETAG) {
		...
		if ((pattrib->ether_type != 0x888e) &&
			...
			SET_TX_DESC_DATA_RATE_FB_LIMIT_8192F(pbuf, 0x1F);
		...
		SET_TX_DESC_RETRY_LIMIT_ENABLE_8192F(pbuf, 1);
		if (pattrib->retry_ctrl == _TRUE)
			SET_TX_DESC_DATA_RETRY_LIMIT_8192F(pbuf, 6);
		else
			SET_TX_DESC_DATA_RETRY_LIMIT_8192F(pbuf, 12);
...
#define RXDMA_IDLE			BIT(17)
	...
	case HW_VAR_TDLS_BCN_EARLY_C2H_RPT:
		rtl8192f_set_BcnEarly_C2H_Rpt_cmd(padapter, *val);
		break;
#endif
	...
	case HW_VAR_WOW_OUTPUT_GPIO: {
		...
		output_value = (pwrctrlpriv->is_high_active == 0) ? 1 : 0;
...
void GetHwReg8192F(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	case HW_VAR_CHK_HI_QUEUE_EMPTY:
		val32 = rtw_read16(padapter, REG_TXPKT_EMPTY_8192F);
		*val = (val32 & BIT(17)) ? _TRUE : _FALSE;
		break;
	case HW_VAR_CHK_MGQ_CPU_EMPTY:
		val16 = rtw_read16(padapter, REG_TXPKT_EMPTY_8192F);
		*val = (val16 & BIT(19)) ? _TRUE : _FALSE;
		break;
#ifdef CONFIG_WOWLAN
...
void rtl8192f_set_hal_ops(struct hal_ops *pHalFunc)
{
	...
	pHalFunc->get_tx_power_index_handler = &PHY_GetTxPowerIndex_8192F;
